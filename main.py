...
def main():
    ...
    parser.add_argument('-d', '--analysis', choices=['bubble', 'min'], help="Displays a bubble graph of"+
                                                                        ...
                                                                        "and happiness")
    parser.add_argument('-mh', '--mosthot', type=int, dest='nhottest', help="show the n most hot countries in asc.  must be greater than 0")
    parser.add_argument('-lh', '--leasthot', type=int, dest='nleasthot', help="show the n most least hot countries in asc.  must be greater than 0")

    parser.add_argument('-mp', '--mosthappy', type=int, dest='nhappiest', help="show the n most happy countries in asc.  must be greater than 0")
    parser.add_argument('-lp', '--leasthappy', type=int, dest='nleasthappy', help="show the n most least happy countries in asc.  must be greater than 0")

    parser.add_argument('-y', '--year', choices=range(2008,2021), help="value to let user piuck year for mh, lh, mp, lp commands")
    ...
    if args.nhottest and args.nhottest > 0:
        print(temp.top_n_hottest_countries(args.nhottest))
    if args.nleasthot and args.nleasthot > 0:
        print(temp.top_n_coldest_countries(args.nleasthot))
    if args.nhappiest and args.nhappiest > 0:
        print(happy.top_n_happiest_countries(args.nhappiest))
    if args.nleasthappy and args.nleasthappy > 0:
        print(happy.top_n_saddest_countries(args.nleasthappy))
